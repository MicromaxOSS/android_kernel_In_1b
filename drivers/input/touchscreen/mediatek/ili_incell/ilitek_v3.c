...
int ili_sleep_handler(int mode)
{
	...
	case TP_DEEP_SLEEP:
		if(ilits->tp_suspend == true)
			ILI_INFO("TP already in deep suspend\n");
		else{
...
//extern int psensor_flag;
//extern int psensor_callflag;
int ili_report_handler(void)
{
	...
	int rlen = 0;
	int tmp = debug_en;
	//if((psensor_flag==0) && (psensor_callflag ==1))
		//return 0;
	...
	if (ilits->input == NULL) {
		ILI_INFO("ignore report request before init\n");
		return 0;
	}
...
static void ili_update_tp_module_info(void)
{
	...
	if (module == 0 || ilits->md_fw_ili_size < ILI_FILE_HEADER) {
		...
		if(ilitek_get_vendor_id() == 0x11)
		{
			ILI_ERR("It's hua xian module,download CTPM_FW_HUAXIAN\n");
			ilits->md_fw_ili = CTPM_FW_HUAXIAN;
			ilits->md_fw_ili_size = sizeof(CTPM_FW_HUAXIAN);				
		}
		else if(ilitek_get_vendor_id() == 0x21)
		{
			ILI_ERR("It's guo xian module,download CTPM_FW_GUOXIAN\n");
			ilits->md_fw_ili = CTPM_FW_GUOXIAN;
			ilits->md_fw_ili_size = sizeof(CTPM_FW_GUOXIAN);
		}
...
static char vendor_id = 0x0;

char ilitek_get_vendor_id(void)
{
	return vendor_id;
}
...
int ili_tddi_init(void)
{
	...
	ili_ic_get_ddi_reg_onepage(0x0, 0xda, &vendor_id, false); 

	ILI_INFO("ili_tddi_init!vendor_id is 0x%x",vendor_id);
