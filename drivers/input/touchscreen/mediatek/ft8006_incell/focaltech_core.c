...
void fts_reset_pin_ctl(int onoff)
{
  FTS_DEBUG("tp reset onoff:%d",onoff);
  if(onoff)
...
static ssize_t sprocomm_read_tp_info(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
	...
	ret = fts_read_reg(FTS_REG_FW_VER,&tpver);
	printk("Sprocomm_TpInfo tpver:0x%x\n",tpver);

	strcpy(tp_module_buffer, "HUAXIAN");
	strcpy(tp_proc_buffer, "FT8006");
