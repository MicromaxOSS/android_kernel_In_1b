...
struct proc_dir_entry *tp_proc_class_entry = NULL;
struct proc_dir_entry *tp_info_entry = NULL;
struct proc_dir_entry *tp_double_tap_enable_entry = NULL;
struct proc_dir_entry *tp_gesture_coordinate_entry = NULL;
struct proc_dir_entry *tp_support_gesture_id_entry = NULL;
struct proc_dir_entry *tp_thresh_entry = NULL;
struct proc_dir_entry *tp_disable_entry = NULL;
struct proc_dir_entry *tp_firmware_update_enable_entry = NULL;
struct proc_dir_entry *fts_test_entry = NULL;

unsigned int double_tap_enable = 0;
unsigned int gesture_report[14]={0};
unsigned char gesture_id[16]={0x01, 0x02, 0x04, 0x05, 0x06, 0x0c, 0x0d};
unsigned int thresh_value = 1;
unsigned int tp_disable_flag = 0;
				  ...
				  struct kobj_attribute *attr, char *buf)
{
    ...
    err = fts_read_reg(FTS_REG_CHIP_ID, &chip_id[0]);
    err = fts_read_reg(FTS_REG_CHIP_ID2, &chip_id[1]);
	err = fts_read_reg(FTS_REG_VENDOR_ID, &vendor_id);
	err = fts_read_reg(FTS_REG_FW_VER,&tpver);
	err = fts_read_reg(FTS_REG_VENDOR_ID,&csver);
	...
	return sprintf(buf, "type:  %s\n" "vendor:  %s\n" "firmware_version:  0x%02x\n" "driver_version:  3.0.0\n", tp_proc_buffer, tp_module_buffer,tpver);
}
...
static struct attribute *tp_info_attrs[] = {
    &tp_info_attr.attr,
    NULL
};

static struct attribute_group tp_info_attr_group = {
    .attrs = tp_info_attrs,
};

static void tp_info_init(void)
{
    int ret = 0;

    printk("%s\n",__func__);

	ret = sysfs_create_group(kernel_kobj, &tp_info_attr_group);

	if(ret)
        printk("failed to create board_properties\n");
}

static ssize_t  double_tap_enable_write(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
    unsigned char temp;
	...
	double_tap_enable = temp == '1'?1:0;
    
	return count;
}

static ssize_t  double_tap_enable_read(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	u32 length = 0;
...
static struct file_operations tp_double_tap_enable_fops = {
	.owner = THIS_MODULE,
	.write = double_tap_enable_write,
	.read = double_tap_enable_read,
};

static int sprocomm_creat_proc_double_tap_enable(void)
{
	tp_double_tap_enable_entry = proc_create("double_tap_enable", 0666, tp_proc_class_entry, &tp_double_tap_enable_fops);
	if (tp_double_tap_enable_entry == NULL)
    {
...
static ssize_t  gesture_coordinate_read(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	u32 length = 0;
    unsigned char nUserTempBuffer[256]; 
    ...
    length = sprintf(nUserTempBuffer, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", 
        gesture_report[0], gesture_report[1], gesture_report[2], gesture_report[3],
        gesture_report[4], gesture_report[5], gesture_report[6], gesture_report[7],
        gesture_report[8], gesture_report[9], gesture_report[10], gesture_report[11],
        gesture_report[12], gesture_report[13]);
...
static struct file_operations tp_gesture_coordinate_fops = {
	.owner = THIS_MODULE,
	.read = gesture_coordinate_read
};

static int sprocomm_creat_proc_gesture_coordinate(void)
{
	tp_gesture_coordinate_entry = proc_create("gesture_coordinate", 0666, tp_proc_class_entry, &tp_gesture_coordinate_fops);
	if (tp_gesture_coordinate_entry == NULL)
    {
        printk("Create gesture_coordinate fail\n");
    }
	return 0;
}

static ssize_t  read_support_gesture_id(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	u32 length = 0;
    unsigned char nUserTempBuffer[256]; 
    ...
    length = sprintf(nUserTempBuffer, "0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x\n",
        gesture_id[0], gesture_id[1], gesture_id[2], gesture_id[3],
		gesture_id[4], gesture_id[5], gesture_id[6]);
...
static struct file_operations tp_support_gesture_id_fops = {
	.owner = THIS_MODULE,
	.read = read_support_gesture_id,
};

static int sprocomm_creat_proc_support_gesture_id(void)
{
	tp_support_gesture_id_entry = proc_create("support_gesture_id", 0666, tp_proc_class_entry, &tp_support_gesture_id_fops);
	if (tp_support_gesture_id_entry == NULL)
    {
        printk("Create support_gesture_id fail\n");
    }
	return 0;
}

static ssize_t  tp_thresh_value_write(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
    unsigned char temp;     
	...
	switch (temp)
	{
		case '0':
			thresh_value = 0;
			fts_write_reg(0x80, 0x7D);
			break;
		case '1':
			thresh_value = 1;
			fts_write_reg(0x80, 0x2D);
			break;
		case '2':
			thresh_value = 2;
			fts_write_reg(0x80, 0x19);
			break;
		default:
			thresh_value = 1;
	}         
...
static ssize_t  tp_thresh_value_read(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	u32 length = 0;
    unsigned char nUserTempBuffer[16]; 
...
static struct file_operations tp_thresh_fops = {
	.owner = THIS_MODULE,
    .write = tp_thresh_value_write,
	.read = tp_thresh_value_read,
};

static int sprocomm_creat_proc_thresh(void)
{
	tp_thresh_entry = proc_create("tp_thresh_value", 0666, tp_proc_class_entry, &tp_thresh_fops);
	if (tp_thresh_entry == NULL)
    {
        printk("Create tp_thresh_value fail\n");
    }
	return 0;
}

static ssize_t  tp_disable_write(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
    unsigned char temp;     
	...
	tp_disable_flag = temp == '1'?1:0;              
    
	return count;
}

static ssize_t  tp_disable_read(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	u32 length = 0;
    unsigned char nUserTempBuffer[16]; 
...
static struct file_operations tp_disable_fops = {
	.owner = THIS_MODULE,
	.write = tp_disable_write,
	.read = tp_disable_read,
};

static int sprocomm_creat_proc_tp_disable(void)
{
	tp_disable_entry = proc_create("psensor_incall_disable_tp", 0666, tp_proc_class_entry, &tp_disable_fops);
	if (tp_disable_entry == NULL)
    {
        printk("Create psensor_incall_disable_tp fail\n");
    }
	return 0;
}

static ssize_t  tp_firmware_update_enable_write(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
    ...
    // fwname = (char *)kzalloc(buflen * sizeof(char), FILE_NAME_LENGTH);
    // if (NULL == fwname) {
        // FTS_ERROR("apk proc wirte buf zalloc fail");
        // return -ENOMEM;
    // }

	if (copy_from_user(fwname, buffer, count))
		return-EPERM;

    //snprintf(fwname, count, "%s", buffer);
	//memcpy(fwname, buffer, FILE_NAME_LENGTH);

    fwname[count - 1] = '\0';

    FTS_INFO("upgrade with bin file through sysfs node");
    printk("upgrade with bin file through sysfs node");
...
static ssize_t  tp_firmware_update_enable_read(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	u32 length = 0;
    unsigned char nUserTempBuffer[16];
	int err = -1;
    ...
    length = sprintf(nUserTempBuffer, "0x%02x\n", tpver);
...
static struct file_operations tp_firmware_update_enable_fops = {
    .write = tp_firmware_update_enable_write,
	.read = tp_firmware_update_enable_read,
};

static int sprocomm_creat_proc_firmware_update_enable(void)
{
	tp_firmware_update_enable_entry = proc_create("firmware_update_enable", 0666, tp_proc_class_entry, &tp_firmware_update_enable_fops);
	if (tp_firmware_update_enable_entry == NULL)
    {
        printk("Create firmware_update_enable fail\n");
    }
	return 0;
}

static ssize_t  fts_test_read(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	u32 length = 0;
    unsigned char nUserTempBuffer[16]; 
...
static struct file_operations fts_test_fops = {
	.owner = THIS_MODULE,
	.write = tp_test_write,
	.read = fts_test_read,
};

static int sprocomm_creat_proc_fts_test(void)
{
	fts_test_entry = proc_create("fts_test", 0666, NULL, &fts_test_fops);
	if (fts_test_entry == NULL)
    {
        printk("Create fts_test fail\n");
    }
	return 0;
}

static int fts_create_proc(void)
{
	tp_proc_class_entry = proc_mkdir(PROC_FTS, NULL);
	
	sprocomm_creat_proc_double_tap_enable();
	sprocomm_creat_proc_gesture_coordinate();
	sprocomm_creat_proc_support_gesture_id();
	sprocomm_creat_proc_thresh();
	sprocomm_creat_proc_tp_disable();
	sprocomm_creat_proc_firmware_update_enable();
	sprocomm_creat_proc_fts_test();
	tp_info_init();
...
extern void disable_power(void);
