...
#define FPSENSOR_PROC_INFO
#ifdef FPSENSOR_PROC_INFO
...
{
	if(bonoff)
		mt_spi_enable_master_clk(g_spi);
	else
		mt_spi_disable_master_clk(g_spi);
}
...
#ifdef FPSENSOR_PROC_INFO
static int subsys_sprocomm_fpsensor_info_read(struct seq_file *m, void *v)
{
	seq_printf(m, "%s\n", "HEDAYUAN:ICNF7332AL");
	return 0;
};

static int proc_sprocomm_fpsensor_info_open(struct inode *inode, struct file *file)
{
	return single_open(file, subsys_sprocomm_fpsensor_info_read, NULL);
};

static const struct file_operations fpsensor_proc_info_fops = {
        .owner = THIS_MODULE,
        .read = seq_read,
        .open = proc_sprocomm_fpsensor_info_open,
};
...
#ifdef FPSENSOR_PROC_INFO
	proc_create("Sprocomm_FPsensorInfo", 0664, NULL, &fpsensor_proc_info_fops);
#endif
