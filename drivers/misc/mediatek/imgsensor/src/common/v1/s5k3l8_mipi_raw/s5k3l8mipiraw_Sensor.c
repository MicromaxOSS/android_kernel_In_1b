...
#define EEPROM_OTP
#ifdef EEPROM_OTP
u16 eeprom_otp_MID_HongShi = 0;		//jiangguijuan add 20180825, HongShi DualCamera 
#endif

/*===FEATURE SWITH===*/
#define FPTPDAFSUPPORT   //for pdaf switch

/*jiangguijuan add,2019-03-11*/
#define DUMP_ARCSOFT_DUALCAM_DATA
static char arcsoft_dualcam_data[2048] = {0};
int save_arcsoft_dualcam_data(bool force);
/*jiangguijuan add,2019-03-11*/
...
static imgsensor_info_struct imgsensor_info = { 
	...
	.checksum_value = 0xba395b5c,		//checksum value for Camera Auto Test
	...
	.cap1 = {							//capture for PIP 15ps relative information, capture1 mode must use same framelength, linelength with Capture mode for shutter calculate
		.pclk = 560000000,				//record different mode's pclk
		.linelength  = 5808,				//record different mode's linelength
		.framelength = 6412,			//record different mode's framelength
		.startx = 0,					//record different mode's startx of grabwindow
		.starty = 0,					//record different mode's starty of grabwindow
		.grabwindow_width  = 4208,		//record different mode's width of grabwindow
		.grabwindow_height = 3120,		//record different mode's height of grabwindow
  ...
  .custom2 = {
		.pclk = 560000000,				//record different mode's pclk
		.linelength = 5808,				//record different mode's linelength
		.framelength =3206, //3168,			//record different mode's framelength
		.startx = 0,					//record different mode's startx of grabwindow
		.starty = 0,					//record different mode's starty of grabwindow
		.grabwindow_width = 4160,		//record different mode's width of grabwindow
		.grabwindow_height = 3120,		//record different mode's height of grabwindow
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] =	 
{
 ...
 { 4160, 3120,	  0,  	0, 4160, 3120, 4160, 3120,   0,	0, 4160, 3120, 	 0, 0, 4160, 3120},// Custom1 (bokeh mode) 
...
static void preview_setting(void)
{
  ...
  write_cmos_sensor(0x6F12, 0x0040);
  write_cmos_sensor(0x6028, 0x4000);    
  write_cmos_sensor(0x0344, 0x0008);	  //0008 2104*1560
  write_cmos_sensor(0x0346, 0x0008);    //0008
  write_cmos_sensor(0x0348, 0x1077);    //1077
  write_cmos_sensor(0x034A, 0x0C37);    //0C37
  write_cmos_sensor(0x034C, 0x0838);    //0838
  write_cmos_sensor(0x034E, 0x0618);    //0618
  ...
  write_cmos_sensor(0x0B00, 0x0000);//0107
...
static void capture_setting(kal_uint16 currefps)
{
  ...
  write_cmos_sensor(0x6F12, 0x0040);
  write_cmos_sensor(0x6028, 0x4000);
  write_cmos_sensor(0x0344, 0x0008);	  //0008 4208x3120
  write_cmos_sensor(0x0346, 0x0008);    //0008
  write_cmos_sensor(0x0348, 0x1077);    //1077
  write_cmos_sensor(0x034A, 0x0C37);    //0C37
  write_cmos_sensor(0x034C, 0x1070);    //1070
  write_cmos_sensor(0x034E, 0x0C30);    //0C30
  ...
  write_cmos_sensor(0x0308, 0x0008);
  write_cmos_sensor(0x0342, 0x16B0);
  write_cmos_sensor(0x0340, 0x190C);
  write_cmos_sensor(0x0202, 0x0200);
  write_cmos_sensor(0x0200, 0x00C6);
  write_cmos_sensor(0x6028, 0x2000);
  write_cmos_sensor(0x602A, 0x0CF4);
...
static void custom1_setting(void)
{
  ...
  write_cmos_sensor(0x0344, 0x0008);	  //0008 4208x3120
  write_cmos_sensor(0x0346, 0x0008);    //0008
  write_cmos_sensor(0x0348, 0x1047);    //1077
  write_cmos_sensor(0x034A, 0x0C37);    //0C37
  write_cmos_sensor(0x034C, 0x1040);    //1070
  write_cmos_sensor(0x034E, 0x0C30);    //0C30
  write_cmos_sensor(0x0900, 0x0011);
  write_cmos_sensor(0x0380, 0x0001);
  write_cmos_sensor(0x0382, 0x0001);
  ...
  write_cmos_sensor(0x6F12, 0x0000);                            
  write_cmos_sensor(0x6F12, 0xFFEB); //need to be calibrated    
  write_cmos_sensor(0x602A, 0x4914);                            
  write_cmos_sensor(0x6F12, 0x0000);                            
  write_cmos_sensor(0x6F12, 0x0002); //need to be calibrated    
  ...
  write_cmos_sensor(0x6F12, 0x4000);                            
  write_cmos_sensor(0x6028, 0x4000);                            
  write_cmos_sensor(0x30C0, 0x0300); //Vsync out from GPIO3     
  write_cmos_sensor(0x30BE, 0x0004); //Vsync in to GPIO4        
...
#ifdef EEPROM_OTP
static kal_int16 s5k3l8_check_eeprom_data(kal_uint32 addr)
{
	kal_uint16 get_byte=0;
	kal_uint32 i2c_speed = 100;
	char pu_send_cmd[2] = {(char)(addr >> 8) , (char)(addr & 0xFF) };

	iReadRegI2CTiming(pu_send_cmd , 2, (u8*)&get_byte,1,0xA0,i2c_speed);
//	printk("s5k3l8_check_eeprom_data get_byte = %d\n",get_byte);
	return get_byte;
}

#endif

#ifdef DUMP_ARCSOFT_DUALCAM_DATA

static int dump_arcsoft_dualcam_data(void)
{	
	kal_uint16 start_addr = 0x0d00;
	kal_uint32 length = 2048;
	int i = 0;
	for(i=0;i<length;i++)
	{
		arcsoft_dualcam_data[i] = s5k3l8_check_eeprom_data(start_addr + i);
	//	printk("%s,---%d--0x%x,	0x%x-----\n",__func__,i,start_addr + i,arcsoft_dualcam_data[i]);
	}
	return 0;	
}
...
int save_arcsoft_dualcam_data(bool force)
{
	...
	if(force)
	{
		dump_arcsoft_dualcam_data();
	}
	fp = filp_open("/sdcard/result.bin",O_RDONLY,0);
	if(IS_ERR(fp) || force)
	{
		printk("/sdcard/result.bin file not exist or force to read\n");
		fp = filp_open("/sdcard/result.bin",O_RDWR | O_CREAT,0644);
		if(IS_ERR(fp))
		{
			printk("create /sdcard/result.bin fail\n");
			return -1;
		}
		oldfs = get_fs();
		set_fs(KERNEL_DS);
		pos = 0;//fp->f_pos;
		vfs_write(fp,arcsoft_dualcam_data,sizeof(arcsoft_dualcam_data),&pos);
		//fp->f_pos = pos;
		set_fs(oldfs);
		filp_close(fp,NULL);
		printk("create /sdcard/result.bin success\n");			
	}
	else
	{
		printk("open success\n");
		printk("/sdcard/result.bin exist,no need to create again\n");
		filp_close(fp,NULL);			
	}
	return 0;
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        spin_unlock(&imgsensor_drv_lock);
        do {
			printk("jiang add s5k3l8 start get_imgsensor_id\n");
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
				printk("wangqRead 3l8 sensor id success, i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);		
				/*jiangguijuan add,2019-03-11*/
				#ifdef DUMP_ARCSOFT_DUALCAM_DATA
				dump_arcsoft_dualcam_data();
				#endif
			...
			printk("wangqRead 3l8 sensor id fail, i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);	
            retry--;
        } while(retry > 0);
        i++;
        retry = 1;
...
/*************************************************************************
* FUNCTION
*	get_imgsensor_otp_info
*
* DESCRIPTION
*	This function get the sensor otp group info
*
* GLOBALS AFFECTED
*
*************************************************************************/
...
{
#ifdef EEPROM_OTP
	if(eeprom_otp_MID_HongShi == 0x72)
		*otp_info = 1;
	else
		*otp_info = 0;
#else
	*otp_info = 0;
#endif
	printk("s5k3l8 get_imgsensor_otp_info group = %d\n", *otp_info);	  
	return ERROR_NONE;
}
#endif
/*************************************************************************
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            LOG_INF("OPEN fail, i2c write id id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
			...
			switch (*feature_data) {
				...
				case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
					*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = 0;
					break;
				case MSDK_SCENARIO_ID_CUSTOM1:
					printk("printk 3l8 sensor of S5k3L8 close_pdaf :%s\n",__func__);
					*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = 0;
					break;
				default:
					*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = 0;
...
UINT32 S5K3L8_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	printk("wang S5K3L8_MIPI_RAW_SensorInit\n");
	return ERROR_NONE;
