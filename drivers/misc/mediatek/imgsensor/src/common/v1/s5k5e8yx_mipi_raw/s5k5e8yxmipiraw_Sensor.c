...
//#define EEPROM_OTP
#ifdef EEPROM_OTP
extern u16 eeprom_otp_MID_HongShi;		//jiangguijuan add 20180825, HongShi DualCamera
#endif
...
{{ 2592, 1944,	  0,	0, 2592, 1944, 1296,  972, 0000, 0000, 1296,  972,	  0,	0, 1296,  972}, // Preview 
 ...
 { 2592, 1944,	  16,  12, 2560, 1920, 2560, 1920, 0000, 0000, 2560, 1920,	  0,	0, 2560, 1920}};	//bokeh
...
{
	//while(1){
		printk("wang read_cmos_sensor(0x0000)=%x\n",read_cmos_sensor(0x0000));
		printk("wang read_cmos_sensor(0x0001)=%x\n",read_cmos_sensor(0x0001));
	//}
...
static void sensor_init(void)
{
  ...
  write_cmos_sensor(0x3C02,0x0F);		//master sync output  
...
static void capture_setting(kal_uint16 currefps)
{
	...
	write_cmos_sensor(0x0344,0x00); //00  	2592x1944
	write_cmos_sensor(0x0345,0x08); //08
	write_cmos_sensor(0x0346,0x00); //00
	write_cmos_sensor(0x0347,0x08); //08
	write_cmos_sensor(0x0348,0x0A); //0A
	write_cmos_sensor(0x0349,0x27); //27
	write_cmos_sensor(0x034A,0x07); //07
	write_cmos_sensor(0x034B,0x9F); //9F
	write_cmos_sensor(0x034C,0x0A); //0A
	write_cmos_sensor(0x034D,0x20); //20
	write_cmos_sensor(0x034E,0x07); //07
	write_cmos_sensor(0x034F,0x98); //98
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {				
				printk("wang S5k5e8yx i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);				
				 #ifdef EEPROM_OTP
				printk("jiang add get_imgsensor_id 5e8 HongShi mid = 0x%x\n",eeprom_otp_MID_HongShi);
			...
			printk("wang S5k5e8yx Read sensor id fail: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
*
* DESCRIPTION
*	This function get the sensor otp info
*
* PARAMETERS
*	*sensorID : return the otp yes or no 
*
* RETURNS
*	None
*
...
static kal_uint32 get_imgsensor_otp_info(UINT32 *otp_info) 
{
	...
	*otp_info = 0;
#endif
	printk("s5k5e8 get_imgsensor_otp_info group = %d\n", *otp_info);	  
	return ERROR_NONE;
}
#endif
/*************************************************************************
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			printk("wang S5k5e8yx Read sensor id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	if (enable) {
		...
		write_cmos_sensor(0x020e, original_dat[0]&0xff);
		write_cmos_sensor(0x020f, original_dat[1]&0xff);
		write_cmos_sensor(0x0210, original_dat[2]&0xff);
		write_cmos_sensor(0x0211, original_dat[3]&0xff);
		write_cmos_sensor(0x0212, original_dat[4]&0xff);
		write_cmos_sensor(0x0213, original_dat[5]&0xff);
		write_cmos_sensor(0x0214, original_dat[6]&0xff);
		write_cmos_sensor(0x0215, original_dat[7]&0xff);
		...
		case SENSOR_FEATURE_GET_CROP_INFO:
			...
			switch (*feature_data_32) {
				...
				case MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO:
					...
					break;
				case MSDK_SCENARIO_ID_SLIM_VIDEO:
				    printk("before MSDK_SCENARIO_ID_SLIM_VIDEO");
					memcpy((void *)wininfo,(void *)&imgsensor_winsize_info[4],sizeof(struct SENSOR_WINSIZE_INFO_STRUCT));
					printk("after MSDK_SCENARIO_ID_SLIM_VIDEO");
					break;
				case MSDK_SCENARIO_ID_CUSTOM1:
...
{
printk("Jeff,enter S5K5E8YX_MIPI_RAW_SensorInit\n");
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	S5K5E8YX_MIPI_RAW_SensorInit	*/
EXPORT_SYMBOL(S5K5E8YX_MIPI_RAW_SensorInit);
