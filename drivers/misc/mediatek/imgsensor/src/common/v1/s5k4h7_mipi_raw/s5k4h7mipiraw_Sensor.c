...
static imgsensor_info_struct imgsensor_info = {
	...
	.pre = {
		.pclk = 273000000,				/* record different mode's pclk */
		.linelength = 3580,				/* record different mode's linelength */
		.framelength = 2542,			/* record different mode's framelength */
...
static void preview_setting(void)
{
	...
	write_cmos_sensor_8(0x0307,0x7E);
	...
	write_cmos_sensor_8(0x0B00, 0x01);//lsc open:0x01,lose:0x00
...
static void capture_setting(kal_uint16 currefps)
{
  ...
  if (currefps == 300) {
	    ...
	    write_cmos_sensor_8(0x0341,0xEE);
		 ...
		 write_cmos_sensor_8(0X0341, 0x69);//3177
	 } else { //15fps
		 write_cmos_sensor_8(0X0340, 0X13);
		 write_cmos_sensor_8(0X0341, 0XDC);//5084
	 }

	write_cmos_sensor_8(0x0342,0x0D);
	write_cmos_sensor_8(0x0343,0xFC);//3580
	write_cmos_sensor_8(0x0200,0x0D);
	write_cmos_sensor_8(0x0201,0x6C);
	write_cmos_sensor_8(0x0202,0x00);
	write_cmos_sensor_8(0x0203,0x02);
...
static void hs_video_setting(void)
{
	...
	write_cmos_sensor_8(0x030D,0x06);
	write_cmos_sensor_8(0x030E,0x00);
	write_cmos_sensor_8(0x030F,0x9A);
	write_cmos_sensor_8(0x3C1F,0x00);
	...
	write_cmos_sensor_8(0x0821,0xC8);
	write_cmos_sensor_8(0x0822,0x00);
	write_cmos_sensor_8(0x0823,0x00);
	write_cmos_sensor_8(0x0112,0x0A);
	write_cmos_sensor_8(0x0113,0x0A);
	...
	write_cmos_sensor_8(0x0341,0x7B);
	write_cmos_sensor_8(0x0342,0x0D);
	write_cmos_sensor_8(0x0343,0xFC);
	write_cmos_sensor_8(0x0200,0x0D);
	write_cmos_sensor_8(0x0201,0x6C);
...
static void slim_video_setting(void)
{
	...
	write_cmos_sensor_8(0x0821,0x5C);
	write_cmos_sensor_8(0x0822,0x00);
	write_cmos_sensor_8(0x0823,0x00);
	write_cmos_sensor_8(0x0112,0x0A);
	write_cmos_sensor_8(0x0113,0x0A);
	...
	do {
	    *sensor_id = return_sensor_id();
	    if (*sensor_id == imgsensor_info.sensor_id) {
			...
			printk("s5k4h7 i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id, *sensor_id, imgsensor_info.sensor_id);
		return ERROR_NONE;
	    }
			printk("s5k4h7 Read sensor id fail, i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id, *sensor_id, imgsensor_info.sensor_id);
	    retry--;
	} while (retry > 0);
	i++;
	retry = 1;
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	sensor_info->SensorModuleName = (char *)"S5K4H7";
	sensor_info->SensorModuleID = 2;
	sensor_info->SensorOtpID = 1;
