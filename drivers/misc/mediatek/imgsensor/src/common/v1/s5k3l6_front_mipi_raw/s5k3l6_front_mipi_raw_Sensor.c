...
#include "s5k3l6_front_mipi_raw_Sensor.h"
/****************************Modify Following Strings for Debug****************************/
#define PFX "S5K3L6_FRONT"
#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)

#define LOG_1 LOG_INF("S5K3L6_FRONT,MIPI 4LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K3L6_FRONT_SENSOR_ID,		//Sensor ID Value: 0x30C8//record sensor id defined in Kd_imgsensor.h
	...
	.pre = {
		...
		.framelength = 3264,			//record different mode's framelength
...
//fps 15
	.cap1 = {							//capture for PIP 15ps relative information, capture1 mode must use same framelength, linelength with Capture mode for shutter calculate
		.pclk = 479555000,				//record different mode's pclk
		.linelength  = 4896,				//record different mode's linelength
		.framelength = 6528,			//record different mode's framelength
		.startx = 0,					//record different mode's startx of grabwindow
		.starty = 0,					//record different mode's starty of grabwindow
		.grabwindow_width  = 4208,		//record different mode's width of grabwindow
		.grabwindow_height = 3120,		//record different mode's height of grabwindow
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x0340, 0x0CC0);
...
static void capture_setting(kal_uint16 currefps)
{
    ...
    if (currefps == 300) {
    	...
    	write_cmos_sensor(0x0100,0x0000);
			...
			write_cmos_sensor(0x0300, 0x0006);
			write_cmos_sensor(0x0342, 0x1320);
			write_cmos_sensor(0x0340, 0x1980);
			write_cmos_sensor(0x38C4, 0x0009);
			write_cmos_sensor(0x38D8, 0x002A);
			write_cmos_sensor(0x38DA, 0x000A);
			write_cmos_sensor(0x38DC, 0x000B);
...
*
*************************************************************************/
extern int sprocomm_get_curr_camera_index(void);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    if(sprocomm_get_curr_camera_index() != IMGSENSOR_SENSOR_IDX_SUB){
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
			...
			printk("s5k3l6 front i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);
                return ERROR_NONE;
            }
			printk("s5k3l6 front Read sensor id fail, i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 1;
...
UINT32 S5K3L6_FRONT_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
    LOG_INF("S5K3L6_FRONT_MIPI_RAW_SensorInit in\n");
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	S5K3L6_FRONT_MIPI_RAW_SensorInit	*/
