...
#ifdef OTP_HongShi
#define OTP_WB_GOLDEN
struct HongShi_S5K5E9_otp_struct {
	int flag;
	int MID;
	int Year;
	int Month;
	...
	int VCM_end;
#ifdef OTP_WB_GOLDEN
	int RGr_ratio_golden;
	int BGr_ratio_golden;
	int GbGr_ratio_golden;
#endif	
}HONGSHI_S5K5E9_OTP;

#define RGr_ratio_Typical 551
#define BGr_ratio_Typical 716
#define GbGr_ratio_Typical 1024

static struct HongShi_S5K5E9_otp_struct s5k5e9_otp;
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.checksum_value = 0x391338c0,

	.pre = {
    .pclk = 188500000,              //record different mode's pclk
		.linelength = 3112,	/* record different mode's linelength */
		.framelength = 2018,         //record different mode's framelength
		...
		.mipi_pixel_rate = 350400000,
		.max_framerate = 300,
	},
	.cap = {
		.pclk = 188500000,
		.linelength = 3112,
		.framelength = 2018, //2035: 30.0019
		.startx =0,
		.starty = 0,
...
write_cmos_sensor_8(0x0305,	0x08);
write_cmos_sensor_8(0x0306,	0x00);
write_cmos_sensor_8(0x0307,	0xAE);
write_cmos_sensor_8(0x030D,	0x08);
write_cmos_sensor_8(0x030E,	0x01);
write_cmos_sensor_8(0x030F,	0x0C);
...
write_cmos_sensor_8(0x0821,	0x67);
...
write_cmos_sensor_8(0x0341, 0xDA);
write_cmos_sensor_8(0x0342, 0x0C);
write_cmos_sensor_8(0x0343, 0x28);
write_cmos_sensor_8(0x0200, 0x0B);
write_cmos_sensor_8(0x0201, 0x9C);
...
void module_info_otp_read(struct HongShi_S5K5E9_otp_struct *S5K5E9_OTP)
{
	unsigned char val = 0;
	unsigned int page = 63;
	
	printk("module_info_otp_read start");
	write_cmos_sensor_8(0x0100,0x01);
	mdelay(10);
	...
	val=read_cmos_sensor_8(0x0A04);//flag of info
	printk("S5K5E9 [read_otp] module info group flag = [0x%x]\n", val);

	if((val&0xc0) == 0x40)
	{
		S5K5E9_OTP->MID = read_cmos_sensor_8(0x0A05);
		S5K5E9_OTP->LID = read_cmos_sensor_8(0x0A09);
		S5K5E9_OTP->Year= read_cmos_sensor_8(0x0A06);
		S5K5E9_OTP->Month= read_cmos_sensor_8(0x0A07);
		S5K5E9_OTP->Day= read_cmos_sensor_8(0x0A08);
		
		S5K5E9_OTP->group = 1;
		printk("[read_otp] module info group1 is valid\n");
		printk("[read_otp] mid = [0x%x], year = [0x%x], month = [0x%x], day = [0x%x]\n", S5K5E9_OTP->MID, S5K5E9_OTP->Year,S5K5E9_OTP->Month,S5K5E9_OTP->Day);
		
		S5K5E9_OTP->RGr_ratio =
		(read_cmos_sensor_8(0x0A0B)<<8)+read_cmos_sensor_8(0x0A0C);
		S5K5E9_OTP->BGr_ratio =
		(read_cmos_sensor_8(0x0A0D)<<8)+read_cmos_sensor_8(0x0A0E);
		S5K5E9_OTP->GbGr_ratio = 
		(read_cmos_sensor_8(0x0A0F)<<8)+read_cmos_sensor_8(0x0A10);
		printk("[read_otp] RGr_ratio = [0x%x], BGr_ratio = [0x%x],GbGr_ratio = [0x%x]\n", S5K5E9_OTP->RGr_ratio, S5K5E9_OTP->BGr_ratio,S5K5E9_OTP->GbGr_ratio);
		
		#ifdef OTP_WB_GOLDEN
		S5K5E9_OTP->RGr_ratio_golden =
		(read_cmos_sensor_8(0x0A11)<<8)+read_cmos_sensor_8(0x0A12);
		S5K5E9_OTP->BGr_ratio_golden =
		(read_cmos_sensor_8(0x0A13)<<8)+read_cmos_sensor_8(0x0A14);
		S5K5E9_OTP->GbGr_ratio_golden = 
		(read_cmos_sensor_8(0x0A15)<<8)+read_cmos_sensor_8(0x0A16);
		printk("[read_otp] RGr_ratio_golden = [0x%x], BGr_ratio_golden = [0x%x], GbGr_ratio_golden = [0x%x]\n", S5K5E9_OTP->RGr_ratio_golden, S5K5E9_OTP->BGr_ratio_golden, S5K5E9_OTP->GbGr_ratio_golden);
		#endif
		
	}else if((val&0x30) == 0x10)
	{
		S5K5E9_OTP->MID = read_cmos_sensor_8(0x0A19);
		S5K5E9_OTP->LID = read_cmos_sensor_8(0x0A1D);
		S5K5E9_OTP->Year= read_cmos_sensor_8(0x0A1A);
		S5K5E9_OTP->Month= read_cmos_sensor_8(0x0A1B);
		S5K5E9_OTP->Day= read_cmos_sensor_8(0x0A1C);
		
		S5K5E9_OTP->group = 2;
		printk("[read_otp] module info group2 is valid\n");
		printk("[read_otp] mid = [0x%x], year = [0x%x], month = [0x%x], day = [0x%x]\n", S5K5E9_OTP->MID, S5K5E9_OTP->Year,S5K5E9_OTP->Month,S5K5E9_OTP->Day);
		
		S5K5E9_OTP->RGr_ratio =
		(read_cmos_sensor_8(0x0A1F)<<8)+read_cmos_sensor_8(0x0A20);
		S5K5E9_OTP->BGr_ratio =
		(read_cmos_sensor_8(0x0A21)<<8)+read_cmos_sensor_8(0x0A22);
		S5K5E9_OTP->GbGr_ratio = 
		(read_cmos_sensor_8(0x0A23)<<8)+read_cmos_sensor_8(0x0A24);
		printk("[read_otp] RGr_ratio = [0x%x], BGr_ratio = [0x%x],GbGr_ratio = [0x%x]\n", S5K5E9_OTP->RGr_ratio, S5K5E9_OTP->BGr_ratio,S5K5E9_OTP->GbGr_ratio);
		
		#ifdef OTP_WB_GOLDEN
		S5K5E9_OTP->RGr_ratio_golden =
		(read_cmos_sensor_8(0x0A25)<<8)+read_cmos_sensor_8(0x0A26);
		S5K5E9_OTP->BGr_ratio_golden =
		(read_cmos_sensor_8(0x0A27)<<8)+read_cmos_sensor_8(0x0A28);
		S5K5E9_OTP->GbGr_ratio_golden = 
		(read_cmos_sensor_8(0x0A29)<<8)+read_cmos_sensor_8(0x0A2A);
		printk("[read_otp] RGr_ratio_golden = [0x%x], BGr_ratio_golden = [0x%x], GbGr_ratio_golden = [0x%x]\n", S5K5E9_OTP->RGr_ratio_golden, S5K5E9_OTP->BGr_ratio_golden, S5K5E9_OTP->GbGr_ratio_golden);
		#endif
	}else
	{
		printk("[read_otp] all of groups are invalid\n");
		S5K5E9_OTP->flag=0x0;
		S5K5E9_OTP->MID =0x00;
		S5K5E9_OTP->LID = 0x00;
		S5K5E9_OTP->Year=0x00;
		S5K5E9_OTP->Month=0x00;
		S5K5E9_OTP->Day=0x00;
		S5K5E9_OTP->group = 0;
		
		S5K5E9_OTP->RGr_ratio = 0;
		S5K5E9_OTP->BGr_ratio = 0;
		S5K5E9_OTP->GbGr_ratio = 0;
		
		#ifdef OTP_WB_GOLDEN
		S5K5E9_OTP->RGr_ratio_golden = 0;
		S5K5E9_OTP->BGr_ratio_golden = 0;
		S5K5E9_OTP->GbGr_ratio_golden = 0;
		#endif
	}
	stop_read_otp();
	LOG_INF("module_info_otp_read end");
}

static int HongShi_S5K5E9_apply_OTP(struct HongShi_S5K5E9_otp_struct *S5K5E9_OTP)
{
	int R_gain,B_gain,Gb_gain,Gr_gain,Base_gain;

	if((S5K5E9_OTP->group != 1) && (S5K5E9_OTP->group != 2))  {
		
		printk("[apply_otp] otp is null\n");
		return -1;
	}
	
#ifdef OTP_WB_GOLDEN
if(S5K5E9_OTP->RGr_ratio_golden && S5K5E9_OTP->BGr_ratio_golden && S5K5E9_OTP->GbGr_ratio_golden)
{
	R_gain = (S5K5E9_OTP->RGr_ratio_golden*1000) / S5K5E9_OTP->RGr_ratio;
	B_gain = (S5K5E9_OTP->BGr_ratio_golden*1000) / S5K5E9_OTP->BGr_ratio;
	Gb_gain = (S5K5E9_OTP->GbGr_ratio_golden*1000) / S5K5E9_OTP->GbGr_ratio;
	printk("[apply_otp] S5K5E9_OTP have golden rgb\n");
}
else
{
	R_gain = (RGr_ratio_Typical*1000) / S5K5E9_OTP->RGr_ratio;
	B_gain = (BGr_ratio_Typical*1000) / S5K5E9_OTP->BGr_ratio;
	Gb_gain = (GbGr_ratio_Typical*1000) / S5K5E9_OTP->GbGr_ratio;	
}
#else	
	R_gain = (RGr_ratio_Typical*1000) / S5K5E9_OTP->RGr_ratio;
	B_gain = (BGr_ratio_Typical*1000) / S5K5E9_OTP->BGr_ratio;
	...
	printk("[apply_otp] R_gain = 0x%x, B_gain = 0x%x, Gb_gain = 0x%x, Gr_gain = 0x%x\n", R_gain, B_gain, Gb_gain, Gr_gain);
	printk("S5K5E9 OTP read HongShi_S5K5E9_apply_OTP read back r_gain_h:%x,r_gain_l:%x,gr_gain_h:%x,gr_gain_l:%x\n",
		read_cmos_sensor_8(0x0210),read_cmos_sensor_8(0x0211),read_cmos_sensor_8(0x020E),read_cmos_sensor_8(0x020F));
	printk("S5K5E9 OTP read HongShi_S5K5E9_apply_OTP read back gb_gain_h:%x,gb_gain_l:%x,b_gain_h:%x,b_gain_l:%x,0x3C0F:%x\n",
		read_cmos_sensor_8(0x0214),read_cmos_sensor_8(0x0215),read_cmos_sensor_8(0x0212),read_cmos_sensor_8(0x0213),read_cmos_sensor_8(0x3C0F));
	return 0;
}
#endif
...
#if 0
static kal_uint32 get_imgsensor_otp_info(UINT32 *otp_info)
{
#ifdef OTP_HongShi
	*otp_info = s5k5e9_otp.group;
#else
	*otp_info = 0;
#endif
	return ERROR_NONE;
...
static kal_uint32 open(void)
{
	...
	#ifdef OTP_HongShi
	HongShi_S5K5E9_apply_OTP(&s5k5e9_otp);
	#endif
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	if (enable) {
		...
		// 0x5E00[1:0]; 00 Color bar, 01 Random Data, 10 Square, 11 BLACK
		write_cmos_sensor_8(0x3200, 0x01);
		write_cmos_sensor_8(0x3462, 0x03);
		write_cmos_sensor_8(0x3230, 0x00);
		write_cmos_sensor_8(0x3290, 0x10);
		write_cmos_sensor_8(0x3201, 0x18);
		write_cmos_sensor_8(0x0b05, 0x01);
		write_cmos_sensor_8(0x0b00, 0x00);
		write_cmos_sensor_8(0x3c60, 0x01);
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_CHECK_SENSOR_ID:
			...
			break;
#if 0
		case SENSOR_FEATURE_GET_OTP_INFO:
			get_imgsensor_otp_info(feature_return_para_32);
			break;
		case SENSOR_FEATURE_GET_CAM_INFO:
			*feature_para = 6;	// 1:Darling; 2:Shinetech; 3:ChengXiang 4:TianShi 5:TXD 6:HongShi 7:"XinChenGuang"
			break;
#endif
